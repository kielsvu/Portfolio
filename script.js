...
window.addEventListener('scroll',function(){
  ...
  if(homeEl){
    var heroH=homeEl.offsetHeight;
    var fadeStart=heroH*.45,fadeEnd=heroH*.90;
    var beamEl=document.getElementById('beam-c');
    if(beamEl){
      var op=s<=fadeStart?1:s>=fadeEnd?0:1-((s-fadeStart)/(fadeEnd-fadeStart));
      beamEl.style.opacity=op;
    }
  }
...
function loadDiary(){
  ...
  fetch('./secret/diary.json?_='+Date.now())
    ...
    .then(function(data){renderDiary(Array.isArray(data)?data:[]);})
...
(function(){
  var el=document.getElementById('foot-views');
  var cnt=document.getElementById('view-count');
  if(!el||!cnt)return;
  fetch('https://api.countapi.xyz/hit/kiel-dev-portfolio/visits')
    .then(function(r){if(!r.ok)throw new Error();return r.json();})
    .then(function(d){
      if(typeof d.value!=='number')throw new Error();
      var target=d.value;
      ...
      (function tick(){
        ...
        cnt.textContent=Math.round(ease*target).toLocaleString();
