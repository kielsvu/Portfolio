...
(function(){
  ...
  function dr(){
    ...
    ctx.save();ctx.beginPath();ctx.moveTo(sx,sy);ctx.lineTo(ex,ey);ctx.strokeStyle='rgba(255,248,255,.18)';ctx.lineWidth=1;ctx.stroke();ctx.restore();
    var tl=OV*2;
    DS.forEach(function(p){
      ...
      ctx.beginPath();ctx.arc(px,py,p.sz,0,6.2832);ctx.fillStyle='rgba(213,197,245,'+p.op+')';ctx.fill();ctx.restore();
    });
